...
* 注释：
*********************************************************************************************/
static void esp32_ExitSerial_call(char *rsp)
{
  if (rsp == NULL) 
  {
  } 
...
static void esp32_ExitSerial(void)
{
  _request_at_3("+++", 100, esp32_ExitSerial_call);
}
...
* 注释：
*********************************************************************************************/
static void esp32_Reset_call(char *rsp)
{
  if (rsp == NULL) 
  {
    //timeout
...
{
  _request_at_3("AT+RESTORE\r\n", 4000, esp32_Reset_call);
}
...
* 注释：
*********************************************************************************************/
static void esp32_Echo_call(char *rsp)
{
  if (rsp == NULL) 
  {
    //timeout
...
static void esp32_GetStatus_call(char *rsp)
{
  ...
  else if (memcmp(rsp, "STATUS:", 7) == 0) 
  {
    ...
    if(status == 0)
      dev_status.networkMode = 0;
    else
      dev_status.networkMode = 1;
...
* 注释：
*********************************************************************************************/
static void esp32_SetNotworkMode_call(char *rsp)
{
  if (rsp == NULL) 
  {
    //timeout
...
static void esp32_SetNotworkMode(void)
{
  _request_at_3("AT+CWMODE=1\r\n", 200, esp32_SetNotworkMode_call);
}

/*********************************************************************************************
* 名称：esp32_SetUartMode
* 功能：
* 参数：
* 返回：
* 修改：
* 注释：
*********************************************************************************************/
static void esp32_SetUartMode_call(char *rsp)
{
  if (rsp == NULL) 
  {
    //timeout
  ...
  {
    dev_status.uartMode = 1;
    end_process_at();
  }
}

static void esp32_SetUartMode(void)
{
  _request_at_3("AT+CIPMODE=1\r\n", 200, esp32_SetUartMode_call);
}

/*********************************************************************************************
* 名称：esp32_OpenSerialMode
* 功能：
* 参数：
* 返回：
* 修改：
* 注释：
*********************************************************************************************/
static void esp32_OpenSerialMode_call(char *rsp)
{
  if (rsp == NULL) 
  {
    //timeout
...
static void esp32_OpenSerialMode(void)
{
  _request_at_3("AT+CIPSEND\r\n", 200, esp32_OpenSerialMode_call);
}
...
static void esp32_GetMAC_call(char *rsp)
{
  ...
  else if (memcmp(rsp, "+CIPSTAMAC:", 11) == 0) 
  {
    ...
    for (int i=0; i<17; i++) 
    {
      if (dev_status.mac[i] == ':' ) continue;
      dev_status.mac[i] = toupper(dev_status.mac[i]);
    }
  }
  else if (memcmp(rsp, "OK", 2) == 0) 
  {
...
static void _esp32_timer_call(void *ptr)
{
  ...
  if(dev_status.init == 0)
  {
    ...
    else if(dev_status.networkMode == 0)
    {
      esp32_SetNotworkMode();
    }
...
#else
    else if (dev_status.uartMode == 0) 
    {
      esp32_SetUartMode();
    }
    if(dev_status.uartMode == 1)
    {
      esp32_OpenSerialMode();
      leds_on(1);
      dev_status.init = 1;
    }
#endif
...
#if !UART_AT_MODE
  if(_response_offset>2)
    esp32_RecvData(_response_buf);
#endif
}
//////////////////////////////////////////////////////
